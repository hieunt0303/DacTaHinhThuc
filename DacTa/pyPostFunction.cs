...
namespace DacTa
{
    public class pyPostFunction : pyNameFunction
    {
        ...
        public void SetStatement(List<string> data_output, string postpath, string namepath)
        {
            ...
            if (vari[1] == "char*")
            {
                string CreateResult = string.Format("\t{0} = ''", vari[0]);
                data_output.Add(CreateResult);
            }
            else if (vari[1] == "R" || vari[1] == "Z")
            {
                string CreateResult = string.Format("\t{0} = 0", vari[0]);
                data_output.Add(CreateResult);
            }
            else if (vari[1] == "B")
            {
                ...
                if (namepath.Contains("Ham")){
                    if(postpath.Contains("VM"))
                        CreateResult = string.Format("\t{0} = True", vari[0]);
                    else
                        CreateResult = string.Format("\t{0} = False", vari[0]);
                }
                else
                    CreateResult = string.Format("\t{0} = True", vari[0]);
                data_output.Add(CreateResult);
                    ...
                    if (conditions[i].Contains("&&") == true)
                    {
                        ...
                        if (conditions_result.Length > 2)
                        {
                            for (int j = 1; j < conditions_result.Length; j++)
                            {
                                ...
                                {
                                    conditions_result[j] = PreWrite(conditions_result[j], 0);
                                    statement += string.Format("and {0}:", conditions_result[j]);
                                }
                                else
                                {
                                    conditions_result[j] = PreWrite(conditions_result[j], 0);
                    ...
                    for (int i = 0; i < conditionVari.Length; i++)   // xử lý từng phần điều kiện i j 
                    {
                        ...
                        if (conditionVari[i].Contains("TH") == true)   // xác định phần i và j bằng  TH
                        {
                            ...
                            if (i == 0)
                            {
                                ...
                                string paramLast = "";
                                if (varipath[2].Length == 3)
                                    paramLast = "-1";
                                string statement1 = string.Format("\tfor {0} in range({1},len(a) {2}):", conVari, varipath[1], paramLast);
                    ...
                    for (int i = 0; i < conditionVari.Length; i++)
                    {
                        ...
                        for (int j = 0; j < vartpath.Length; j++)
                        {
                            ...
                            if (vartpath[j].Contains("TH") == true)
                            {
                                ...
                                if (checkVari[1].Length == 3)
                                    paramLast = "-1";
                                string statement1 = string.Format("\tfor {0} in range({1},len(a) {2}) :", conVari, checkVari[0], paramLast);
                                data_output.Add(statement1);
                                //data_output.Add("\t\t\t{");
